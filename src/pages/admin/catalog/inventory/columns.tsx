...
export function getColumns(): ColumnDef<Product>[] {
  return [
    ...
    },
    {
      ...
      header: "Shtrix-kod",
      cell: ({ row }) => (
        <span className="text-sm text-muted-foreground">{row.original.barcode ?? "—"}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      header: ({ column }) => <DataTableColumnHeader column={column} title="Yangilandi" />,
      cell: ({ row }) =>
        new Date(row.original.updated_at).toLocaleString("uz-UZ", {
          day: "2-digit",
          month: "2-digit",
          year: "numeric",
          hour: "2-digit",
